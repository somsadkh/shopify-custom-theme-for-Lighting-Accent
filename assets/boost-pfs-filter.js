...
console.log(boostPFSFilterConfig);
...
(function() {
	...
	ProductGridItem.prototype.compileTemplate = function(data) {
		...
		itemHtml = itemHtml.replace(/{{itemGridWidthClass}}/g, itemGridWidthClass);
      	...
      	// Add hover class
		var itemHoverClass = images.length > 1 ? 'product-card-hover' : '';
      ...
      var secondImageUrl = images.length > 1 ? Utils.optimizeImage(images[1]['src'], '600x600') : '';
      var secondImage = images.length > 1 ? '<img src="' +secondImageUrl+ '"  class="product-second-image"/>' : '';
		itemImagesHtml += '%;">';
		itemImagesHtml += '<img id="' + imgId + '" ' +
			'class="grid-view-item__image lazyload product-first-image" ' +
			'src="' + Utils.getFeaturedImage(images, '300x300') + '" ' +
			'data-src="' + Utils.getFeaturedImage(images, '{width}x') + '" ' +
			'data-widths="[180, 360, 540, 720, 900, 1080, 1296, 1512, 1728, 2048]" ' +
			'data-aspectratio="' + aspect_ratio + '" ' +
			'data-sizes="auto" ' +
			'alt="{{itemTitle}}">' + secondImage;
		itemImagesHtml += '</div>';
		itemImagesHtml += '</div>';
	...
	function buildPrice(data, onSale, priceVaries) {
		...
		if (boostPFSConfig.custom.vendor_enable) {
			...
			priceHtml += '</dt>';
			priceHtml += '<dd><a href="'+ Utils.buildProductItemVendorUrl(data.vendor) +'">';
